#!/bin/bash
# slurm-vllm-serve.sh — Single or Multi-node vLLM job launcher.
# This should be used for single or multinode jobs using mp.
#
#SBATCH --export=ALL
...
# starts vLLM with proper NCCL/Ray configuration, and runs monitors.
...
(
    ...
    echo "[serve-0] vllm head srun process $IVLLM_HEAD_PID on $IVLLM_HEAD_NODE_IP"
