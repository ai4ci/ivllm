import { copyFileSync, mkdirSync, writeFileSync, readFileSync, existsSync, rmSync } from 'fs';
 ...
 */
export function makeRemoteOps(config: Credentials, mode: OpsMode = 'real'): RemoteOps {
  if (mode === 'real') {
    return {
...
function buildDryRunOps(): RemoteOps {
  return {
    ...
    async matchVllmVersion(minVllmVersion) {
      return selectBestVersion(['0.22.0', minVllmVersion], minVllmVersion) ?? '0.22.0';
    },
    async checkSSH() {
...
class MockRemoteFs {
  ...
  async exec(
    ...
    try {
      const stdout = execSync(wrapped, { encoding: 'utf-8', timeout: 5000 }).trim();
      return { exitCode: 0, stdout };
    } catch (e: unknown) {
