...
Possible choices: `auto`, `deepseek_v32`, `deepseek_v4`, `hf`, `inkling`, `mistral`, `slow`
...
To swap the Rust BPE backend that powers HF fast tokenizers for the [fastokens](https://web.archive.org/web/20260730113243/https://github.com/crusoecloud/fastokens) implementation, set `VLLM_USE_FASTOKENS=1` instead — that override applies to any mode that loads an HF fast tokenizer (`hf`, `deepseek_v32`, `deepseek_v4`, …).
...
Possible choices: `eager`, `lazy`, `prefetch`, `torchao`, `None`
...
Note: [TPU](https://web.archive.org/web/20260730113243/https://docs.vllm.ai/projects/tpu/en/latest/) platform only supports Ray for distributed inference.
...
Possible choices: `aiter`, `auto`, `cutlass`, `deep_gemm`, `deep_gemm_mega_moe`, `emulation`, `flashinfer_b12x`, `flashinfer_cutedsl`, `flashinfer_cutlass`, `flashinfer_trtllm`, `flydsl`, `hpc`, `humming`, `marlin`, `triton`, `triton_unfused`
...
API docs: https://docs.vllm.ai/en/latest/api/vllm/config/#vllm.config.DiffusionConfig
...
Default: `KernelConfig(ir_op_priority=IrOpPriorityConfig(rms_norm=[], fused_add_rms_norm=[]), enable_flashinfer_autotune=None, enable_cutedsl_warmup=True, enable_bf16x3_router_gemm=False, moe_backend='auto', linear_backend='auto')`
