...
#      artifacts, and are they left group-writable even when the SOURCE
#      files themselves aren't (mirrors the copyDirectory permission fix:
...
sandbox_run_test "capture_job_diagnostics_forces_group_writable_even_from_restrictive_source" compute '
    create_status_pending "diag-job" "model" 30 > /dev/null 2>&1
    update_status_initialise "diag-job"
    job_dir=$(resolve_job_dir "diag-job")

    # Source log file deliberately has NO group-write bit — same scenario
    # as a fresh git-clone engine file (see the copyDirectory fix).
    echo "log line one" > "$job_dir/vllm.0.log"
    chmod 644 "$job_dir/vllm.0.log"
